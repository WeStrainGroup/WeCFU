...
hf auth login          # 新版命令；老教程里的 huggingface-cli login 已废弃
```
...
4. 复制 token，粘到终端（输入时不显示字符，正常）
5. 问 `Add token as git credential?` 时输 **Y**
...
**Q：协作者上传时报"Per-session limit is 100 images"？**
A：每个会话最多 100 张、400 MB。让他分批上传，或者每次跑完导出 zip 后让他刷新页面（清空 session）。
...
> https://huggingface.co/spaces/&lt;你的用户名&gt;/wecfu
>
> 把图片拖进去就行。每次会话上限 100 张图 / 400 MB，1 小时不动会自动清空。
> 详细操作打开右上 Help 按钮。
